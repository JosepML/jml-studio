...
// Historial del chat: vive mientras la pestaña esté abierta (se reinicia al
// volver a entrar en Asistente), no hace falta persistirlo.
let historialChat = [];
...
export async function renderAsistente(container) {
  ...
  // Gastos por categoría del año en curso (suma simple del importe, sin
  // prorratear amortizaciones — suficiente como contexto orientativo para el
  // chat, no se usa para ningún cálculo fiscal oficial).
  const gastosPorCategoriaAnio = {};
  ...
  const contextoChat = {
    fecha_hoy: hoyIso,
    anio,
    trimestre_actual: qActual,
    modelo130_pct_configurado: cfg.modelo130_pct,
    cuota_autonomo_mensual: cfg.cuota_autonomo_importe,
    mes_actual: { facturado_transferencia: resumenMes.transferencia, facturado_efectivo: resumenMes.efectivo },
    anual: {
      facturado_transferencia: resumenAnual.transferencia,
      facturado_efectivo: resumenAnual.efectivo,
      gastos_deducibles: resumenAnual.gastosDeducibles,
      gastos_no_deducibles: resumenAnual.gastosNoDeducibles,
      gastos_totales: resumenAnual.gastosTotales,
      beneficio_fiscal: resumenAnual.beneficioFiscal,
      beneficio_real: resumenAnual.beneficioReal,
    },
    trimestre_actual_datos: {
      facturado_transferencia: tActual.transferencia,
      gastos_deducibles: tActual.gastosDeducibles,
      provision_modelo130_estimada: provision.aIngresar,
    },
    facturacion_por_mes: MESES_CORTO.map((m, i) => ({ mes: m, facturado: porMesInsight[i] })),
    gastos_por_categoria_anio: gastosPorCategoriaAnio,
    clientes_top: topClientes.map(([nombre, importe]) => ({ nombre, importe })),
    ticket_medio_por_proyecto: ticketMedio,
    num_alertas_pendientes: alertas.length,
  };
  ...
  // --- Chat financiero ---
  historialChat = [];
  const $chatMensajes = container.querySelector("#chat-mensajes");
  ...
  async function enviarPregunta() {
    ...
    pintarChat();
    const idxPensando = historialChat.push({ rol: "asistente", texto: "Pensando…", pensando: true }) - 1;
    pintarChat();
    try {
      const respuesta = await preguntarAsistenteFinanciero(pregunta, contextoChat, historialChat.slice(0, idxPensando));
      historialChat[idxPensando] = { rol: "asistente", texto: respuesta };
    } catch (e) {
      historialChat[idxPensando] = { rol: "asistente", texto: "Error: " + e.message, error: true };
    }
    pintarChat();
  }

  container.querySelector("#chat-enviar").addEventListener("click", enviarPregunta);
