...
import { getConfig } from "./config-usuario.js";
...
// solos: cada vez que abre la app se comprueba si falta alguno de TODO el año
// en curso (no solo hasta el mes actual, sino también los meses que aún no
// han llegado) y se crea con el importe que tenga configurado en
// Configuración, para que no tenga que volver a añadirlos a mano ni un solo
// mes — incluidos los que quedan por venir.
function plantillas(cfg) {
  return [
    {
      // Mismo texto todos los meses (no lleva el mes en el concepto).
      concepto: "Gestoría - honorarios mensuales",
      conceptoConMes: false,
      importe: cfg.gestoria_importe,
      ivaSoportado: cfg.gestoria_iva_soportado,
      categoria: "servicios",
    },
    {
      ...
      conceptoConMes: true,
      importe: cfg.cuota_autonomo_importe,
      ivaSoportado: 0,
      categoria: "fijo",
    },
  ];
...
// este año — desde agosto (mes índice 7) hasta diciembre, no solo hasta hoy.
// Enero-julio no se tocan porque ya tienen los gastos reales reconciliados a
// mano y no deben duplicarse con estas plantillas. Se puede llamar en cada
// arranque de la app: no duplica nada porque primero mira qué meses ya
// existen.
const PRIMER_MES_AUTOGENERADO = 7; // agosto (0 = enero)

export async function asegurarGastosFijosMensuales() {
  const anio = new Date().getFullYear();
  const cfg = getConfig();
  const PLANTILLAS = plantillas(cfg);
  ...
  for (const plantilla of PLANTILLAS) {
    for (let mes = PRIMER_MES_AUTOGENERADO; mes <= 11; mes++) {
