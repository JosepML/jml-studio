...
assert.match(gastos, /ivaDeduciblePorDefecto = CATEGORIAS_GASTO\[categoria\]\?\.ivaDeduciblePctDefecto/, "La importación debe usar la regla de deducibilidad de la categoría");
assert.doesNotMatch(gastos, /#g-iva-pct"\)\.value = Number\.isFinite\(Number\(datos\.iva_porcentaje\)\)/, "El tipo de IVA no debe escribirse como porcentaje deducible");
